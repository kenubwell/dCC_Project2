...
class Robot:
    ...
    def robot_attack(self, targeted_dinosaur): 
        print('\nSelect which weapon you want to use for your attack:')
        print('Press "0" for "Laser Beam"')
        print('Press "1" for "Arm Blast"')
        user_select = int(input('Press "2" for "Missles": '))
        if user_select >= 0 and user_select <= 2:
            if user_select == 0:
                self.weapon = Weapon('Laser', 65)
            elif user_select == 1:
                self.weapon = Weapon('Blast', 55)
            elif user_select == 2:
                self.weapon = Weapon('Missle', 50) 
        elif user_select > 2 or user_select < 0:
            user_select = int(input('Invalid entry. Re-select a weapon to attack with: '))
